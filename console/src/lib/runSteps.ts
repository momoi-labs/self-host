...
/**
 * The phase a run is in, read from the step it is on: a create is creating,
 * then starting, then provisioning, then ready. The Operator reads a phase
 * where a badge has room for one word; the step list has the steps.
 */
const PHASES: Record<string, string> = {
  ...
  "system-packages": "Provisioning",
  "user-and-ssh": "Provisioning",
  mise: "Provisioning",
  tools: "Provisioning",
  setup: "Provisioning",
  checks: "Provisioning",
...
/** What an action is doing before it has named a step. */
...
export function phase(action: string, step: string | null | undefined): string {
  return (step && PHASES[step]) || DOING[action] || action;
}
...
/**
 * The steps an action walks, each with a state, from the record's own view of
 * the operation rather than from the log: what is behind the step it is on
 * is done, what is ahead is pending, and the step itself is running, or
 * failed when the operation is. For a bar drawn where the log is not at
 * hand, a table cell. A step the sequence does not know leaves every
 * segment pending, which the bar reads as not started.
 */
export function barSteps(
  action: string,
  step: string | null | undefined,
  failed = false,
): { key: string; label: string; state: StepState }[] {
  const at = position(action, step)?.at ?? 0;
  return (SEQUENCES[action] ?? []).map((name, index) => ({
    key: name,
